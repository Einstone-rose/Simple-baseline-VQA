...
class ImageEmbedding(nn.Module):
    def __init__(self, image_channel_type='I', output_size=1024, mode='train',
        ...
        for param in self.extractor.parameters():  # 禁止这些参数进行训练
        ...
        self.extractor.classifier = nn.Sequential(*extactor_fc_layers) # 这里的意思是以元组的形式传进参数中
        ...
        self.mode = mode
        self.extract_features = extract_features  # False
        self.features_dir = features_dir  # False
    ...
    def forward(self, image, image_ids):
        ...
        print("The image_embedding are finished...")
...
class VQAModel(nn.Module):
    ...
    def __init__(self, vocab_size=10000, word_emb_size=300, emb_size=1024, output_size=1000, image_channel_type='I', \
                 ques_channel_type='lstm', use_mutan=True, mode='train', extract_img_features=False, features_dir=None):
    ...
    def forward(self, images, questions, image_ids):
        ...
        ques_embeddings = self.ques_channel(self.word_embeddings(questions))
        # The image_embeddings size is:  torch.Size([28, 1024])
        # The ques_embeddings size is:  torch.Size([28, 1024])
        if hasattr(self, 'mutan'):
            combined = self.mutan(ques_embeddings, image_embeddings)
        else:
            # assert ques_embeddings.size(1)==image_embeddings.size(1)
