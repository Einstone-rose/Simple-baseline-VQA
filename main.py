...
from vqa import VQAModel as VQAModel
...
parser.add_argument('-c', '--config', type=str, default='config_vqa_sgd.yml')

# 这个函数主要返回dataloader,question_data,answer_data
def load_datasets(config, phases):
    ...
    datafiles = {x: '{}.pkl'.format(x) for x in phases}  # datafile = {'train': 'train.pkl', 'val': 'val.pkl'}
...
def main(config):
    ...
    # config['model']['params']['features_dir'] = os.path.join(config['data']['dir'], config['data']['test']['emb_dir'])
    # 定义模型和损失函数
    ...
    if 'reload' in config['model']:
        ...
        if os.path.exists(pathForTrainedModel): # 不存在
...
if __name__ == '__main__':
    ...
    args.config = "config/config_vqa_sgd.yml"
    config = yaml.load(open(args.config)) # config是一个字典
    config['use_gpu'] = (config['use_gpu'] and torch.cuda.is_available()) # 都满足的时候
